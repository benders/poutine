...
describe("selectBestSource", () => {
  ...
  it("local FLAC vs peer FLAC at equal bitrate → local wins", () => {
    const local = src({ remoteId: "local-flac", format: "flac", bitrate: 1000, sourceKind: "local" });
    const peer = src({ remoteId: "peer-flac", format: "flac", bitrate: 1000, sourceKind: "peer", peerId: "peer-a" });
  ...
  it("local MP3 320 vs peer FLAC 1000 → peer wins (format quality dominates)", () => {
    const local = src({ remoteId: "local-mp3", format: "mp3", bitrate: 320, sourceKind: "local" });
    const peer = src({ remoteId: "peer-flac", format: "flac", bitrate: 1000, sourceKind: "peer", peerId: "peer-a" });
    expect(selectBestSource([local, peer])).toBe(peer);
  });

  it("requestedFormat exact match wins over higher-quality non-match", () => {
    const flac = src({ remoteId: "flac", format: "flac", bitrate: 1000, sourceKind: "local" });
    const mp3 = src({ remoteId: "mp3", format: "mp3", bitrate: 320, sourceKind: "local" });
  ...
  it("higher bitrate wins when format is the same", () => {
    const low = src({ remoteId: "low", format: "mp3", bitrate: 128, sourceKind: "local" });
    const high = src({ remoteId: "high", format: "mp3", bitrate: 320, sourceKind: "local" });
    expect(selectBestSource([low, high])).toBe(high);
  });

  it("null format sources still rank (below known formats)", () => {
    const known = src({ remoteId: "mp3", format: "mp3", bitrate: 128, sourceKind: "local" });
    const unknown = src({ remoteId: "unk", format: null, bitrate: 500, sourceKind: "local" });
