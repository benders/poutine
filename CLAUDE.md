...
- **Two-hub federation test stack: `docker-compose.federation.yml`** — spins up hub-a (3001) + hub-b (3002) each with their own Navidrome on isolated internal networks, plus a shared `federation` network so the hubs can reach each other by service name. Music paths injected via `FED_MUSIC_A`/`FED_MUSIC_B` env vars (default: committed fixture MP3s in `test/federation/fixtures/`). Run with `pnpm test:federation`; override paths for real-music local testing.
- **Navidrome scans on startup, not on demand** — with `ND_SCANSCHEDULE=0`, Navidrome runs one scan at startup then stops. The federation test driver polls `POST /admin/sync` in a retry loop until `local.trackCount > 0` rather than using a fixed sleep, so it adapts to both the small fixture files and large real libraries.
...
- `docker-compose.federation.yml` — two-hub test stack for federation regression testing; hub-a (3001) + hub-b (3002), each with their own Navidrome. Music paths from `FED_MUSIC_A`/`FED_MUSIC_B` env vars.
