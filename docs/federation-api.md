...
- **Invitee-side accept** (`POST /api/admin/hub/peers/accept`, admin API, not wire): the mirror direction of the handshake check. If the *inviter's* id is locally tombstoned, the invitation's `issued_at` must postdate that tombstone's `created_at` or the accept is refused with `403 {"error": "Inviter is tombstoned by this hub"}` before any network call is made. Postdated → the tombstone row is deleted and the inviter's mirror row is upserted; the upsert flips `tombstoned → active` but leaves a locally `disabled` inviter disabled — accepting an invitation does not override local policy.
