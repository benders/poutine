...
`/api/version` (#196) is the cheap sibling: no Navidrome ping, safe to poll per open SPA tab (60 s + window focus). `buildId` is a short SHA-256 of the on-disk `${PUBLIC_DIR}/index.html` (`hub/src/services/spa-build-id.ts`, mtime+size-cached) — Vite content-hashes every chunk name into index.html, so *any* rebuild changes it, no `APP_VERSION` bump or hub restart needed. Sentinels: `"dev"` (no `PUBLIC_DIR` — Vite dev server owns updates), `"unknown"` (index.html unreadable). The SPA (`frontend/src/components/UpdateNotifier.tsx`) treats any other change from its boot-time baseline as an update: it auto-reloads when safe (paused, or casting to Sonos — the device keeps playing), defers behind an "Update available" banner while playing locally, snapshots the queue/position to sessionStorage across the reload (`frontend/src/lib/player-snapshot.ts`, 30 s freshness window), and never auto-reloads twice for the same buildId (reload-loop guard).
...
- **Token**: bare `instance_*.remote_id` — the Navidrome id. No prefix, no encoding. Collision across unrelated Navidromes is negligible. Navidrome 0.64+ ids are 22-char base62. The 0.64 migration (#283) leaves hash-derived album/artist ids unchanged, but re-encodes legacy 32-hex MD5 album ids from pre-BFR (< 0.55) libraries — Share IDs copied before such an upgrade stop resolving.
...
- **Version tracks `latest` unless `NAVIDROME_IMAGE` is set** (#283). Upstream releases can change Subsonic output under us — 0.64.0 made `coverArt` conditional and re-encoded every Navidrome id; see [pitfalls.md](pitfalls.md#navidrome-ops). Run `NAVIDROME_IMAGE=deluan/navidrome:<ver> pnpm test:federation` before moving a deployment to a new release. **0.63 → 0.64 upgrade:** Navidrome's one-way migration re-encodes most track ids (verified: unified ids, stars, play counts and album Share IDs survive, since they key on metadata hashes / unchanged album ids). Until the hub re-syncs, `track_sources` points at dead remote ids and streams fail — locally until auto-sync sees the post-upgrade scan (one full sync), on peers until their next poll (≤ 6 min). Back up `navidrome-data` first; the migration has no down path. Details: #283, error relay: #285.
