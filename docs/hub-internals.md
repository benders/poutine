...
- **Merge hardening (#242, Phase 1).** `runMergePipeline(db, { logger })` (`hub/src/library/merge-pipeline.ts`) wraps `mergeLibraries()` with a post-merge orphan audit and is the call site every sync path (`sync.ts`, `auto-sync.ts`, `routes/admin.ts` peer-data reset) now goes through instead of calling `mergeLibraries` directly. `auditOrphans(db)` (`hub/src/library/orphan-audit.ts`) scans `user_stars` (all three `kind`s), `playlist_tracks`, and `play_events` for target/track ids with no matching row in the now-rebuilt `unified_*` tables, returning counts + up-to-10 sample ids per category; a non-zero total is logged as a warning. Phase 1 is report-only — it does not remap or preserve orphaned rows across a merge; that lands in Phase 2.
