...
| `getNowPlaying`   | Implemented     | **Per-user by design** (#237): the caller sees only their own active players — stock Subsonic returns a server-wide list; the cross-user view is admin-only (`/api/admin/hub/activity/active`). Fed by `scrobble?submission=false` pings, one entry per (user, client), in-memory only, expiring 5 min after the last ping. Entries carry the full Song plus `username`, `minutesAgo`, `playerId`, `playerName`. |
...
| `scrobble`  | Implemented     | Records a per-user play in `play_events` (#197). `submission=true` (default) counts; `submission=false` is a now-playing notification — no durable play, but it updates the in-memory now-playing registry behind `getNowPlaying` and the admin Activity page (#237; the SPA pings on play/resume/track-change and every 2 min while playing). Accepts one or many `id`; unknown/malformed ids are skipped so a batch still records the rest. Honors optional `time` (epoch ms) to backfill a play's timestamp; absent, stamps now. This is the *only* play-recording path: the SPA scrobbles for both local and Sonos-cast playback off the actual playback position. |
