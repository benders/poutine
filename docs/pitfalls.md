...
| Adding a "last admin" guard to admin demotion or deletion                              | #274      | Not needed and not wanted: both endpoints reject `id === request.userId`, so the acting admin always survives. A count-based guard would only add a race with no case that reaches it |
