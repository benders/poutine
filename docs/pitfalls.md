...
| Renaming/moving `hub/src/library/merge-worker.ts`                                                                                                                                                       | #242      | `runMergePipelineAsync` resolves both a dev/test `.ts` path and a prod compiled `.js` path from `import.meta.url` — check both branches when the file moves. Keep the worker dependency-light (no fastify/routes/services imports): it must load standalone under tsx (dev) and as compiled JS (prod) |
...
| Assuming `v=` in a Subsonic request is the client's release version                                               | #255      | `v=` is the protocol version (fixed `1.16.1`, never changes across releases). The SPA embeds its own build in `c=` instead (`c=poutine/x.y.z`, `frontend/src/lib/subsonic.ts`) — Activity History's `streamClientLabel` ignores `clientVersion` for this reason. This only affects the SPA's own `c=`; `proxy.ts`/`federation.ts` hardcode fixed internal names (`poutine-proxy`/`poutine-federation`) when forwarding to Navidrome, so it doesn't touch the `REAL_PATH_CLIENTS` pinning in `navidrome-native.ts` (see Navidrome ops below). |
| Parsing boolean query params with `=== "false"`                                                                    | #237      | Non-JS clients capitalize: py-sonic sends `submission=False`. Java servers use `Boolean.parseBoolean` (case-insensitive) — match that (`/^(false\|0)$/i`), or the param silently flips meaning (a now-playing ping recorded as a durable play). Only the federation compat suite catches it. |
| Piping a streamed response with bare `.pipe()` instead of `stream/promises` `pipeline()`                           | #263      | A client abort fires neither `end` nor `error` on the source: the upstream fetch drains to completion into a dead socket (full-file byte counts for a one-second listen) and any per-stream bookkeeping keyed on those events leaks forever. `pipeline()` propagates the disconnect both ways — see `routes/subsonic/stream.ts` + `routes/federation.ts`; `StreamTrackingService` also closes out orphaned rows at boot as a crash backstop. |
...
| Deleting a user row without re-homing `instances.owner_id`                             | #274      | `instances.owner_id REFERENCES users(id)` is `NOT NULL` with **no** `ON DELETE CASCADE`, and instance rows are owned by an arbitrary user (`SELECT id FROM users LIMIT 1` at admission). `DELETE /api/admin/hub/users/:id` reassigns owned rows to the acting admin inside the delete transaction — otherwise the delete throws `SQLITE_CONSTRAINT` and 500s. `playlists` / `user_stars` / `play_events` do cascade. The column is write-only dead weight; #275 drops it and this workaround with it |
| Demoting or deleting the `POUTINE_OWNER_USERNAME` account                              | #274      | Refused by `loadTargetUser({ protectOwner })`. There is no way back: `seedOwner` re-seeds only when the `users` table holds no real accounts, and `reset-password.sh` errors on a missing user. Worse, the DLNA browse path authenticates as that account's u+p (`HubSubsonicCaller` with no `asUser`), so deleting it makes every Browse fail Subsonic auth → `routes/dlna.ts` catches and serves an empty container: the whole library silently disappears behind one `warn` line. Promoting it back to admin is still allowed — that is the recovery path |
| Letting anything create a user named `__system__`                                      | #274      | The placeholder name is load-bearing: `GET /users` hides it and every user mutation refuses it. A real account with that name would be invisible in the admin UI, undeletable through the API, and uncounted by `seedOwner`'s "no real users" check. `POST /users` reserves it |
| Relying on the self-target check alone to keep one admin alive                         | #274      | `requireOwner` reads `is_admin` in the **preHandler**, so two admins can both be authorized before either write lands and then demote or delete each other — leaving zero admins and no reachable `requireOwner` route. Both handlers call `otherAdminCount()` in the handler body, synchronously with the write, so the second request observes the first |
...
| Expecting `song.path` in Subsonic responses to be the on-disk path                            | #252      | Default is a tag-derived **virtual** path (`Artist/Album/Title.ext`) — zero folder signal. `reportRealPath` is per-player, **pinned into the player record** (keyed user+client+UA) at creation. The hub provisions it at runtime over the Navidrome **native API** (`services/navidrome-native.ts`: `/auth/login` + `PUT /api/player/:id`) for the `poutine-sync` and `poutine-proxy` records, at boot and before each local sync. That API is unversioned/internal, so provisioning is best-effort: breakage = one warning + virtual paths + folder-report coverage % drop, never an outage. Don't reintroduce `ND_SUBSONIC_DEFAULTREPORTREALPATH` or client-name bumps — provisioning supersedes both |
