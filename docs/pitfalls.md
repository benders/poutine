...
| Signing anything user-facing with the Ed25519 federation key                            | #272      | That key is cluster trust — it admits and evicts peers. User invitations sign under `settings.user_invite_key` (HMAC-SHA-256) instead. New secret-bearing feature? Mint it a key; don't borrow one with a different blast radius (cf. `castSecret` vs `internalAuthSecret`) |
| Storing an invite/reset token in the DB rather than its hash                            | #272      | `user_invitations` keeps `sha256(token)` only, so a DB leak yields nothing redeemable. It matters more here than most places: `users.password_enc` is reversible, so the DB is already the prize |
| Comparing an ISO-8601 timestamp column against SQLite `datetime('now')`                 | #272      | `datetime('now')` renders `2026-09-03 15:04:05`; a JS `toISOString()` value sorts ABOVE it on the same instant (space < 'T'), so `expires_at > datetime('now')` treats every row as unexpired. Compare against `strftime('%Y-%m-%dT%H:%M:%fZ','now')` and write timestamps in that shape |
| Adding a new unauthenticated route outside `/login`                                     | #272      | The SPA route must fire zero authenticated fetches on mount, and both `App`'s `checkAuth` guard and `apiFetch`'s 401 redirect need the path added — otherwise 401 → redirect → mount → 401. Public API routes also need a rate limit; `/api/invites/*` carries a per-IP fixed window |
